...
function createTrigger(m) {
  ...
  try {
    var trigger = ScriptApp.newTrigger('update').timeBased().everyMinutes(m).create();
  } catch (e) {
    e = (typeof e === 'string') ? new Error(e): e;
    var msg =  Utilities.formatString('Exception ocurred while creating a trigger');
...
function _deleteTriggers(triggerID) {
  try {
    ...
    e = (typeof e === 'string') ? new Error(e): e;
    var msg =  Utilities.formatString('Exception ocurred while deleting triggers. id: %s', triggerID);
    PersistLog.severe(msg);
    throw e;
  }
}
